...
import { MessageSquarePlus, Send, Tag } from "lucide-react";
        ...
        {empty ? (
          ...
          posts.map((p, i) => (
            <motion.div
              ...
              className="glass-card-hover p-5 rounded-3xl"
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <span className="px-3 py-1 rounded-full border border-border/40 bg-secondary/20 text-xs">{p.subject}</span>
              </div>
              <p className="mt-3 whitespace-pre-wrap leading-relaxed text-sm">{p.body}</p>
            </motion.div>
          ))
        )}
      </div>
