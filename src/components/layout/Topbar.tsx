...
import { doc, onSnapshot, query, collection, where, limit, onSnapshot as onSnap2 } from "firebase/firestore";
...
const Topbar = ({ onMenuToggle }: TopbarProps) => {
  ...
  useEffect(() => {
    if (!user) return;
    const q = query(collection(db, "notifications"), where("toUid", "==", user.uid), where("read", "==", false), limit(50));
  ...
  const level = Number(stats?.level || levelFromXp(xp));
    ...
    <header className="sticky top-0 z-30 w-full glass-card border-b border-border/30 px-4 md:px-6 py-3">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button onClick={() => navigate("/focus/pomodoro")} className="glow-button gap-2 text-sm h-9 px-4" size="sm">
