...
const Pomodoro = () => {
  const { user } = useAuth();
  const { mode, timeLeft, isRunning, startPomodoro, pause, reset, setTimeLeft } = useFocus();
 ...
 useEffect(() => {
  ...
  // Auto: when countdown reaches 0 while running, save focus + switch phase
  useEffect(() => {
    if (!isRunning) return;
    ...
    if (!isBreak) {
      ...
      // auto start break? if you want: startPomodoro(breakMins)
    } else {
      // finished break -> back to focus
      setIsBreak(false);
      setTimeLeft(focusMins * 60);
      // auto start focus? if you want: startPomodoro(focusMins)
    }
  ...
  const onToggle = () => {
    ...
    // mode is pomodoro but paused -> resume by re-starting with current timeLeft
    // easiest resume: set running true by calling startPomodoro with current remaining mins is messy,
    // so we just startPomodoro only if timeLeft matches current config.
    // better: keep resume() in context. If you want, I can add it.
    startPomodoro(Math.max(1, Math.ceil(timeLeft / 60)));
  };
  ...
  const onEndAndSave = async () => {
    ...
    // how many minutes studied in THIS phase
    ...
    <div className={`max-w-2xl mx-auto space-y-6 transition-all duration-500 ${deepWork ? "scale-105" : ""}`}>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="glass-card p-8 rounded-2xl flex flex-col items-center">
            ...
            <circle
              ...
              style={{
                filter: `drop-shadow(0 0 8px ${
                  isBreak ? "hsl(var(--warning) / 0.4)" : "hsl(var(--primary) / 0.4)"
                })`,
              }}
            />
          </svg>
        ...
        <div className="flex items-center gap-4">
          <button onClick={onReset} className="p-3 rounded-xl bg-secondary/30 hover:bg-secondary/50 text-muted-foreground hover:text-foreground transition-all">
          ...
          <button onClick={onEndAndSave} className="p-3 rounded-xl bg-secondary/30 hover:bg-secondary/50 text-muted-foreground hover:text-foreground transition-all">
